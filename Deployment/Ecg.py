...
class ECG:
    def __init__(self):
        ...
        self.transformer_class_names = None
        self.transformer_sequence_len = None
    ...
    def DimensionalReduciton(self,test_final):
        ...
        model_path = base_dir / "PCA_ECG (1).pkl"
    ...
    def ModelLoad_predict(self,final_df):
        ...
        base_dir = Path(__file__).resolve().parent
        model_path = base_dir / "Heart_Disease_Prediction_using_ECG (4).pkl"
    ...
    def load_transformer_checkpoint(self, checkpoint_name="checkpoint_best.pt"):
        ckpt_path = self.base_dir / checkpoint_name
        if not ckpt_path.exists():
            raise FileNotFoundError(f"Không tìm thấy checkpoint: {ckpt_path}")

        checkpoint = torch.load(ckpt_path, map_location=self.device)

        configs_raw = checkpoint.get("configs", None)
        hparams_raw = checkpoint.get("hparams", None)
        state_dict = checkpoint.get("model", None)

        if configs_raw is None or hparams_raw is None or state_dict is None:
            raise ValueError(
                "checkpoint_best.pt không có đủ các key 'configs', 'hparams', 'model'."
            )

        configs = SimpleNamespace(**configs_raw)
        model = ecgTransForm(configs=configs, hparams=hparams_raw).to(self.device)
        model.load_state_dict(state_dict)
        model.eval()

        self.transformer_model = model
        self.transformer_class_names = getattr(configs, "class_names", None)
        self.transformer_sequence_len = getattr(configs, "sequence_len", None)

    def preprocess_signal_for_transformer(self, ecg_1dsignal):
        x = np.array(ecg_1dsignal, dtype=np.float32)
        ...
        # pad hoặc cắt theo sequence_len của checkpoint
        if self.transformer_sequence_len is not None:
            if len(x) < self.transformer_sequence_len:
                pad_len = self.transformer_sequence_len - len(x)
                x = np.pad(x, (0, pad_len), mode="constant")
            elif len(x) > self.transformer_sequence_len:
                x = x[:self.transformer_sequence_len]

        # đổi sang [B, C, L] = [1, 1, L]
        x = torch.tensor(x, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
        x = x.to(self.device)
        return x

    def ModelLoad_predict_transformer(self, ecg_1dsignal):
        if self.transformer_model is None:
            self.load_transformer_checkpoint("checkpoint_best.pt")
